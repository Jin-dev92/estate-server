...
// 비즈니스 외 경로(문서 /docs·메트릭 /metrics 등)는 0으로 두어 노이즈·비용을
// 줄인다. Prometheus가 /metrics를 15s마다 긁어 트레이스 노이즈·비용이 크므로
// 제외한다.
// (헬스체크 등이 생기면 EXCLUDED에 추가. CORS preflight는 미사용이라 제외 안 함.)
const EXCLUDED_PATHS = ['/docs', '/metrics']; // '/docs-json'도 includes('/docs')로 함께 잡힌다
