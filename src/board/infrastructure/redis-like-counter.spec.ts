...
function createMockRedis() {
  const pipeline = { set: jest.fn(), get: jest.fn(), exec: jest.fn() };
...
describe('RedisLikeCounter', () => {
  ...
  describe('getMany', () => {
    ...
    it('키별 GET 파이프라인 결과를 숫자 맵으로 바꾸고 미스(null)는 제외한다', async () => {
      // exec는 [error, value] 튜플의 배열을 돌려준다.
      ...
      expect(redis._pipeline.get).toHaveBeenCalledWith('board:like:count:p1');
      expect(redis._pipeline.get).toHaveBeenCalledWith('board:like:count:p2');
