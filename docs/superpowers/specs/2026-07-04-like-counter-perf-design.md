...
- **TTL 부여**(drift 자가 치유): 만료되면 다음 조회가 COUNT로 재구축한다. 값은 **3600s**로 확정(상수로 명시). `redis-board-cache`의 안전망 TTL(120s)보다 긴 이유: 카운터는 쓰기마다 INCR/DECR로 계속 갱신되는 반면 캐시는 무효화가 안전망이라 짧아야 하고, 카운터의 TTL은 drift 치유가 유일한 목적이라 길어도 된다.

### 2.2 포트 / 어댑터
- 새 포트 `LIKE_COUNTER` (`src/board/application/like-counter.ts`):
  ```
  increment(postId): Promise<void>      // 키가 "존재할 때만" +1 + TTL 갱신 (Lua 원자)
  decrement(postId): Promise<void>      // 키가 "존재할 때만" -1 + TTL 갱신 (Lua 원자)
  getMany(postIds): Promise<Map<postId, number>>   // MGET. 미스는 맵에서 제외
  backfill(entries: Map<postId, number>): Promise<void>  // SET NX(+TTL) 일괄 — 미스 채움
  ```
...
> **⚠️ TTL 치유의 한계 — 활성 글(sliding TTL):** increment/decrement Lua가 매 증감마다 `EXPIRE`로 TTL을 다시 3600s로 **슬라이딩**한다. 따라서 좋아요/취소가 꾸준한 인기 글은 키가 사실상 만료되지 않아 "TTL 만료 → COUNT 재구축" 경로가 **작동하지 않는다**. 위 표의 "TTL 치유"는 활동이 멎어 3600s가 지나는 조용한 글에만 성립한다. 만약 best-effort 증감이 조용히 실패(현재 `Logger.warn`만, 메트릭 없음)해 drift가 한 번 생기면, 그 활성 글은 활동이 멈출 때까지 무기한 drift로 남을 수 있다. 좋아요 카운트의 낮은 리스크에선 수용하되, 후속 강화 옵션: (a) 증감은 TTL을 갱신하지 않고 backfill만 TTL을 설정해 활성 글도 ≤1h마다 재구축되게 하거나, (b) best-effort 실패를 카운터/Sentry로 가시화. (PR #85 리뷰 게이트 지적 반영)
